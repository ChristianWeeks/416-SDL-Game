  ...
  channels() 
{
  ...
  music = Mix_LoadMUS("sound/ASmallAdventure.mp3");
  // Need to install midi to play the following:
  // music = Mix_LoadMUS("sound/ballad2.mid");
  if (!music) throw string("Couldn't load ASmallAdventure.mp3")+Mix_GetError();

  startMusic();
  sounds.push_back( Mix_LoadWAV("sound/hiss.wav") );
